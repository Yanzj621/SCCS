...
class Decoder(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False,
        ...
        # self.convds3 = nn.Conv3d(n_filters * 8, n_classes, kernel_size=1)
        # self.convds2 = nn.Conv3d(n_filters * 4, n_classes, kernel_size=1)
        # self.convds1 = nn.Conv3d(n_filters * 2, n_classes, kernel_size=1)

        # self.projection_head = nn.Sequential(
        #     nn.AdaptiveAvgPool3d((1, 1, 1)),  # 全局平均池化
        #     nn.Flatten(),  # 展平
        #     nn.Linear(n_filters, 32),  # 假定VNet输出512个特征通道
        #     nn.ReLU(),
        #     nn.Linear(32, 32)
        # )
        # self.projector = nn.Sequential(nn.Conv3d(n_filters, n_filters, kernel_size=1),
        #                            nn.ReLU(),
        #                            nn.Conv3d(n_filters, n_filters, kernel_size=1))
    ...
    def forward(self, features):
        ...
        size = out_seg.shape[2:]
        out = dict()
        # out['project'] = self.projector(x9)
        # out['project_map'] = F.interpolate(out['project'], size=size, mode='trilinear',
        #                                    align_corners=False)
        # out['level3'] = F.interpolate(self.convds3(x5_up), size=size, mode='trilinear', align_corners=False)
        # out['level2'] = F.interpolate(self.convds2(x6_up), size=size, mode='trilinear', align_corners=False)
        # out['level1'] = F.interpolate(self.convds1(x7_up), size=size, mode='trilinear', align_corners=False)
        out['me'] = x5
        out['fin'] = x9
        out['output'] = out_seg
...
# class fea_gen(nn.Module):
#     def __init__(self, cin):
#         super(fea_gen, self).__init__()
#         self.projector = nn.Sequential(nn.Conv3d(16, 16, kernel_size=1, bias=False),
#                                        nn.BatchNorm3d(16),
#                                        nn.PReLU(),
...
#         x = self.projector(x)
#         x = F.interpolate(x, size=(112, 112, 80), mode='trilinear', align_corners=False)
#         return x
