...
class iRMB(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, norm_in=True, has_skip=True, exp_ratio=1.0, norm_layer='bn_2d',
                 act_layer='relu', v_proj=True, dw_ks=3, stride=1, dilation=1, se_ratio=0.0, dim_head=16, window_size=7,
...
class Encoder(nn.Module):
    def __init__(self, params, drop_out):
...
class Decoder(nn.Module):
    def __init__(self, params):
        ...
        self.projector = nn.Sequential(
            nn.Conv2d(16, 16, kernel_size=1),  # Adapted for 16 input channels
            nn.PReLU(),
            nn.Conv2d(16, self.ft_chns[0], kernel_size=1)  # Output dimension is project_dim
        )
        #
        # # Adding deep supervision for multiple levels
        self.convds3 = nn.Conv2d(self.ft_chns[3], self.n_class, kernel_size=1)
        self.convds2 = nn.Conv2d(self.ft_chns[2], self.n_class, kernel_size=1)
        self.convds1 = nn.Conv2d(self.ft_chns[1], self.n_class, kernel_size=1)
    ...
    def forward(self, feature):
        ...
        x_last = self.up4(x_3, x0)

        project = self.projector(x_last)

        # Deep supervision outputs
        ds3_output = self.convds3(x_1)
        ds2_output = self.convds2(x_2)
        ds1_output = self.convds1(x_3)
        ...
        out = {
            'project': project,
            'project map':F.interpolate(project, size=size, mode='bilinear', align_corners=False),
            'level3': F.interpolate(ds3_output, size=size, mode='bilinear', align_corners=False),
            'level2': F.interpolate(ds2_output, size=size, mode='bilinear', align_corners=False),
            'level1': F.interpolate(ds1_output, size=size, mode='bilinear', align_corners=False),
            'output': output
        }
...
class UNet_2d(nn.Module):
    def __init__(self, in_chns, class_num,drop_out):
        ...
        params = {'in_chns': in_chns,
                  ...
                  'acti_func': 'relu'}
        if drop_out == False:
            self.encoder = Encoder(params,drop_out = False)
        else:
            self.encoder = Encoder(params,drop_out = True)
        self.decoder = Decoder(params)
    ...
    def forward(self, x):
        ...
        # features = self.cont_c1_a(features)
        # features = F.relu(features)
        # features = self.cont_c1_b(features)

        # pos_features = features[pos_indices]
        # neg_features = features[neg_indices]
        #
        # # Calculate contrastive loss (simplified version)
        # loss = contrastive_loss(pos_features, neg_features)  # Define this function separately
