...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, features_a, features_b, mask):
        # 提取特征并调整维度以适应余弦相似度计算
        # features_a = features_a.mean(dim=(-3, -2, -1))
        # features_b = features_b.mean(dim=(-3, -2, -1))
        mask = F.interpolate(mask.unsqueeze(1).float(), size=(7,7,5), mode='trilinear', align_corners=False)
        features_a = features_a * mask
        features_b = features_b * mask

        features_a = features_a.mean(dim=(-3, -2, -1))  # 平均每个通道
        features_b = features_b.mean(dim=(-3, -2, -1))



        features = torch.cat([features_a, features_b], dim=0)
        features = torch.stack([features[i] for i in range(0, len(features), 2)] +
                               [features[i] for i in range(1, len(features), 2)], dim=0)
        # 计算余弦相似度矩阵
        similarity_matrix = self.cosine_similarity_matrix(features)
    ...
    def cosine_similarity_matrix(self, features):
        # L2归一化
        features_norm = F.normalize(features, p=2, dim=1)  # 归一化通道维度
    ...
    def contrastive_loss(self, similarity_matrix):
        ...
        labels = torch.arange(0, N, dtype=torch.long).to(similarity_matrix.device)
...
#             self.class_weights = nn.Parameter(torch.tensor(class_weights, dtype=torch.float32), requires_grad=False)
#
#     def prob_forward(self, pred1, pred2, mask=None):
#         size = pred1.size()
#         N, nclass = size[0], size[1]
#         # N x C x H x W
#         pred1_one_hot = pred1.view(N, nclass, -1)
#         pred2_one_hot = pred2.view(N, nclass, -1)
#         # target = target.view(N, 1, -1)
#         # target_one_hot = to_one_hot(target.type(torch.long), nclass).type(torch.float32)
#
#         # N x C x H x W
#         inter = pred1_one_hot * pred2_one_hot
#         union = pred1_one_hot + pred2_one_hot
#
#         if mask is not None:
#             mask = mask.view(N, 1, -1)
#             inter = (inter.view(N, nclass, -1) * mask).sum(2)
...
#         return 1 - dice.mean()
#
#     def forward(self, logits1, logits2, mask=None):
#         size = logits1.size()
#         N, nclass = size[0], size[1]
#
#         logits1 = logits1.view(N, nclass, -1)
#         logits2 = logits2.view(N, nclass, -1)
#         # target = target.view(N, 1, -1)
#
#         pred1, nclass = get_probability(logits1)
#         pred2, nclass = get_probability(logits2)
#
#         # N x C x H x W
#         pred1_one_hot = pred1
#         pred2_one_hot = pred2
...
class CrossEntropyLoss(nn.Module):
    ...
    # def softmax_mse_loss(input_logits, target_logits, loss_mask):
...
#     #     img_l = 1
#     #     patch_l = 0.5
#     input_for_softmax = F.softmax(input_logits*(1-loss_mask), dim=1)
#     target_for_softmax = F.softmax(target_logits*(1-loss_mask), dim=1)
#     input_back_softmax = F.softmax(input_logits*(loss_mask), dim=1)
#     target_back_softmax = F.softmax(target_logits * (loss_mask), dim=1)
#     mse_for_loss = (input_for_softmax-target_for_softmax)**2
#     mse_back_loss = (input_back_softmax - target_back_softmax) ** 2
#     mse_loss = (mse_for_loss + mse_back_loss)*0.5
#     return mse_loss
...
def softmax_kl_loss(input_logits, target_logits):
    ...
    kl_div = torch.sum(F.kl_div(input_log_softmax, target_softmax, reduction='none'), dim=1)
