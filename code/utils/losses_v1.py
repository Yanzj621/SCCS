...
class contra_loss(torch.nn.Module):
    def __init__(self, temperature=0.5, batch_size=2, num_class=2):
    ...
    def contrastive_loss(self,matrix):
        labels = torch.arange(self.num).to(matrix.device)
        loss = F.cross_entropy(matrix, labels)
        return loss.mean()

    def forward(self, feature1,feature2):
        feature1 = feature1.view(feature1.size(0), -1)
        feature2 = feature2.view(feature2.size(0), -1)
        matrix = torch.matmul(F.normalize(feature1, dim=1), F.normalize(feature2, dim=1).T) / self.temperature
        loss = self.contrastive_loss(matrix)
        return loss
...
class BoundaryLoss(nn.Module):
    def __init__(self, n_classes, weight=0.1, delta=0.1):
    ...
    def forward(self, y_true, y_pred, mask):
        ...
        y_true = self._one_hot_encoder(y_true.unsqueeze(1))
        mask = self._one_hot_mask_encoder(mask)


        diff_pre = torch.abs((y_true - y_pred)*mask)


        # 计算边界区域的损失
        boundary_loss = torch.exp(-(self.delta - diff_pre) / self.delta)
        # e_mask = torch.exp(p_mask)
        # 加权
        weighted_loss = self.weight * boundary_loss
        # sum = weighted_loss.sum()

        # 返回损失值
        return weighted_loss.sum() / (mask.sum() + 1e-8)  # 可以根据需要调整如何计算平均值
